    ...
    def update(self, image, tlwhs, det_scores=None):
        ...
        if self.classifier is None:
            ...
            if self.use_tracking:
                ...
                if len(tracks) > 0:
                    ptlbrs = np.asarray([track.tlbr for track in tracks], dtype=np.float32)
                    ptlbrs = self.corr_tracker.predict(self.im0, image, ptlbrs)
                    p_tlwhs = ptlbrs.copy()
                    p_tlwhs[:, 2:] -= p_tlwhs[:, :2]
                    for pi, ptk in enumerate(tracks):
                        ptk._tlwh = np.asarray(p_tlwhs[pi], dtype=np.float)
